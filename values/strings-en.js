const strings = {
    appTitle: 'TileSlider',
    appLongName: 'The Tile Slider Game',
    btnStart: 'Start',
    btnHelp: 'Help',
    btnAbout: 'About',
    btnStats: 'Statistics',
    btnConfig: 'Settings',
    btnOk: 'Ok',
    btnCancel: 'Cancel',
    configboxTitle: 'Settings',
    configboxPlaySoundsLabel: 'Play sounds:',
    configboxSwitchStyleLabel: 'Tile style:',
    configboxColorSchemeLabel: 'Tile scheme:',
    statsboxTitle: 'Statistics',
    statsboxGamesLabel: 'Games:',
    gamesPlayed: 'Played',
    gamesEnded: 'Ended',
    gamesWon: 'Won',
    statsboxGraphLabel: 'Performance Graph:',
    helpTitle: 'How to Play',
    helpMessage: `The goal of this game is to <b>PUT IN ORDER</b> all the tiles.<br><br>
        You can only move the pieces to the only available space.<br><br>
        The game will end when all the pieces are arranged.<br><br>
        <b>Try to solve it within the least number of steps!</b>`,
    aboutTitle: 'About the Game',
    aboutMessage: `© Copyright 2021 by Erick Levy<br><br>
        This is a remake of an old simple game coded first in Java for Android,
        and ported to <b>HTML5, CSS3 and Javacript</b> for my web skills test area.<br><br>
        Thank you for taking the time to learn about and play with this little app.`,
    restartTitle: 'Game Incomplete',
    restartMessage: 'This action will restart the game. Are you sure?',
    lblSteps: '? steps...',
    gameOverTitle: 'Game Over',
    gameOverScore: 'You have achieved it in ? steps...<br><br>',
    gameOverWin: 'Congratulations!',
    gameOverTry: 'You can do it better!',
    pressStart: 'Press the START button...'
}